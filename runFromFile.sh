...
echo "=================================================="
ffmpeg -i "$INPUT_PATH" -i "${FILE_NAME}_en.srt" -i "${FILE_NAME}_kr.srt" -c copy -c:s mov_text -map 0 -map 1:s -map 2:s -metadata:s:s:0 language=eng -metadata:s:s:0 handler_name="English" -metadata:s:s:1 language=kor -metadata:s:s:1 handler_name="Korean" "${FILE_NAME}_final.mp4" -y
